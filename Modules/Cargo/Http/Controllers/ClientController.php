...
class ClientController extends Controller
{
    ...
    public function create()
    {
        ...
        $adminTheme = env('ADMIN_THEME', 'adminLte');
        return view('cargo::'.$adminTheme.'.pages.clients.create')->with(['branches' => $branches,'customPriceLists' => $customPriceLists,]);
    }
    ...
    public function edit($id)
    {
     ...
     //   $adminTheme = env('ADMIN_THEME', 'adminLte');return view('cargo::'.$adminTheme.'.pages.clients.edit')->with(['model' => $client, 'branches' => $branches, 'packages' => $packages,'customPriceLists' => $customPriceLists]);
     return view('cargo::adminLte.pages.clients.edit', [
        'model' => $client,
        'branches' => $branches,
        'packages' => $packages,
        'customPriceLists' => $customPriceLists,
        'client' => $client, // تأكد من تمرير $client هنا
    ]);
    }
    ...
    public function update(ClientRequest $request, $id)
    {
        ...
        $client = Client::findOrFail($id);
        $client->custom_price_list_id = $request->input('custom_price_list_id');
        $client->save();
        $data = $request->only(['def_mile_cost','def_return_mile_cost','def_mile_cost_gram','def_return_mile_cost_gram','def_return_cost_gram','def_insurance_gram','def_tax_gram','def_shipping_cost_gram','def_return_cost','def_insurance','def_tax','def_shipping_cost','supply_cost','pickup_cost','how_know_us','package_access_options','follow_up_mobile','follow_up_country_code' , 'follow_up_name','name', 'email', 'password', 'responsible_mobile','country_code' ,'responsible_name','national_id','branch_id','address']);
