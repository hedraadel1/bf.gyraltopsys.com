...
class ShipmentController extends Controller
{
    ...
    public function create()
    {
        ...
        $customPriceLists = CustomPriceList::all();
        $client = Client::findOrFail(1);

        $adminTheme = env('ADMIN_THEME', 'adminLte');
        return view('cargo::' . $adminTheme . '.pages.shipments.create')->with(['customPriceLists' => $customPriceLists,
        'client' => $client,]);
    }
        ...
        if ($covered_cost != null) {
            if ($weight > 1) {
                if (ShipmentSetting::getVal('is_def_mile_or_fees') == '2') {
                    ...
                    $shipping_cost_for_extra = (float) ($def_shipping_cost_gram * ($weight - 2));
                    ...
                    $shipping_cost_for_extra = (float) ($def_mile_cost_gram * ($weight - 2));
                }
                $insurance = (float) $def_insurance ?? $covered_cost->insurance + (float) ($def_insurance_gram * ($weight - 2));
            ...
            if ($weight > 1) {
                if (ShipmentSetting::getVal('is_def_mile_or_fees') == '2') {
                    $return_cost = $def_return_cost + (float) ($def_return_cost_gram * ($weight - 2));
                    $shipping_cost_first_one = $def_shipping_cost + $package_extras;
                    $shipping_cost_for_extra = (float) ($def_shipping_cost_gram * ($weight - 2));
                } else if (ShipmentSetting::getVal('is_def_mile_or_fees') == '1') {
                    $return_cost = $def_return_mile_cost + (float) ($def_return_mile_cost_gram * ($weight - 2));
                    $shipping_cost_first_one = $def_mile_cost + $package_extras;
                    $shipping_cost_for_extra = (float) ($def_mile_cost_gram * ($weight - 2));
                }

                $insurance = $def_insurance + (float) ($def_insurance_gram * ($weight - 2));
