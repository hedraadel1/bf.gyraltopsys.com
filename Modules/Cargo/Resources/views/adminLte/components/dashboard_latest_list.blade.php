@php
    ...
    $count = Modules\Cargo\Entities\ShipmentSetting::getVal('latest_shipment_count')
        ? Modules\Cargo\Entities\ShipmentSetting::getVal('latest_shipment_count')
        : 10;
    if ($user_role == $admin || $user_role == $staff) {
        ...
        if ($user_role == $admin || auth()->user()->can('manage-drivers')) {
            $captains = Modules\Cargo\Entities\Driver::withCount([
                'transaction AS wallet' => function ($query) {
        ...
        $branch_id = Modules\Cargo\Entities\Branch::where('user_id', auth()->user()->id)
            ->pluck('id')
            ->first();
        $shipments = Modules\Cargo\Entities\Shipment::where('branch_id', $branch_id)
            ->limit($count)
            ->orderBy('id', 'desc')
            ->get();
        $captains = Modules\Cargo\Entities\Driver::where('branch_id', $branch_id)
            ->withCount([
                'transaction AS wallet' => function ($query) {
                    $query->select(DB::raw('SUM(value)'));
                },
        ...
        $client_id = Modules\Cargo\Entities\Client::where('user_id', auth()->user()->id)
            ->pluck('id')
            ->first();
        $shipments = Modules\Cargo\Entities\Shipment::limit($count)
            ->orderBy('id', 'desc')
                    ...
                    <tbody>
                        @foreach ($shipments as $key => $shipment)
                            <tr>
                                ...
                                <td width="5%"><a
                                        href="{{ route('shipments.show', $shipment->id) }}">{{ $shipment->code }}</a></td>
                                <td>{{ $shipment->getStatus() }}</td>
                                {{--    <td>{{$shipment->type}}</td> --}}
                                <td>
                                    @if (in_array($user_role, [$admin, $auth_branch]) || auth()->user()->can('manage-customers'))
                                        <a
                                            href="{{ route('clients.show', $shipment->client_id) }}">{{ $shipment->client->name ?? 'No Client Name' }}</a>
                                    @else
                            ...
                            @endif --}}
                                ...
                                <td>{{ format_price($shipment->tax + $shipment->shipping_cost + $shipment->insurance) }}
                                </td>
                    ...
                    <tbody>
                        @foreach ($captains as $key => $captain)
                            ...
                            @if ($captain->wallet > 0 ?? 0)
                                <tr>
                                    <td><a href="{{ route('drivers.show', $captain->id) }}">{{ $captain->code }}</a>
                                    </td>
                                    <td>{{ $captain->name }}</td>
                        ...
                        @foreach ($captains as $key => $captain)
                            @php
                                $count = Modules\Cargo\Entities\ShipmentSetting::getVal('latest_shipment_count')
                                    ? Modules\Cargo\Entities\ShipmentSetting::getVal('latest_shipment_count')
                                    : 10;
                                $shipments = Modules\Cargo\Entities\Shipment::where('captain_id', $captain->id)
                                    ->limit($count)
                                    ->orderBy('id', 'desc')
                                    ->get();
                            @endphp
                            @foreach ($shipments as $key => $shipment)
                                <tr>
                                    <td width="5%"><a
                                            href="{{ route('shipments.show', $shipment->id) }}">{{ $shipment->code }}</a>
                                    </td>
                                    <td>{{ $shipment->getStatus() }}</td>
                                    <td>{{ $shipment->type }}</td>
                                    <td>
                                    ...
                                    @if ($user_role != $auth_branch)
                                        @if (in_array($user_role, [$admin]) || auth()->user()->can('manage-branches'))
                                            <td><a
                                                    href="{{ route('branches.show', $shipment->branch_id ?? 0) }}">{{ $shipment->branch->name ?? 'No Branch Name' }}</a>
                                            </td>
                                        @else
                                            <td>{{ $shipment->branch->name ?? 'No Branch Name' }}</td>
                                        @endif
    ...
    <div class="col-md-12">
        <div class="card card-custom card-stretch">
            ...
            <div class="card-body">
                ...
                @php
                    $missions = Modules\Cargo\Entities\Mission::where('captain_id', $driver_id)
                        ->whereNotIn('status_id', [
                            \Modules\Cargo\Entities\Mission::DONE_STATUS,
                            \Modules\Cargo\Entities\Mission::CLOSED_STATUS,
                        ])
                        ->where('due_date', Carbon\Carbon::today()->format('Y-m-d'))
                        ->get();
                @endphp
                            ...
                            @foreach ($missions as $key => $mission)
                                <tr style="background-color:tomatom">
                                    ...
                                    @php
                                        ...
                                        $mission_cost = $helper->calcMissionShipmentsAmount(
                                            $mission->getRawOriginal('type'),
                                            $mission->id,
                                        );
                                    @endphp
                                    ...
                                    <td>
                                        <div style="width: 55%;height: 30px;border: 1px solid;border-radius: 3px;">
                                        </div>
                                    </td>
                    ...
                    <tbody>
                        ...
                        @php
                            $count = Modules\Cargo\Entities\ShipmentSetting::getVal('latest_shipment_count')
                                ? Modules\Cargo\Entities\ShipmentSetting::getVal('latest_shipment_count')
                                : 10;
                            $missions = Modules\Cargo\Entities\Mission::limit($count)
                                ->orderBy('id', 'desc')
                                ->where('captain_id', $driver_id)
                                ->whereNotIn('status_id', [
                                    \Modules\Cargo\Entities\Mission::DONE_STATUS,
                                    \Modules\Cargo\Entities\Mission::CLOSED_STATUS,
                                ])
                        ...
                        @foreach ($missions as $key => $mission)
                            <tr>
                                <td width="5%"><a
                                        href="{{ route('missions.show', $mission->id) }}">{{ $mission->code }}</a></td>
