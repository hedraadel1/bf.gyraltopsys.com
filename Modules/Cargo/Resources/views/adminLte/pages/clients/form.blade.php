...
<div class="form-group">
    <label for="custom_price_list_id">قائمة الاسعار : </label>
    <select name="custom_price_list_id" id="custom_price_list_id" class="form-control">
        <option value="">قائمة الاسعار الافتراضية</option>
        @foreach ($customPriceLists as $priceList)
            <option value="{{ $priceList->id }}" {{ $client->custom_price_list_id == $priceList->id ? 'selected' : '' }}>{{ $priceList->name }}</option>
        @endforeach
    </select>
</div>
